...
/// Validates that a path is under the workspace root (reads root from env).
///
/// # Errors
///
/// Returns `Err` with a message if the path is not under the workspace root.
...
///
/// Returns `Ok(())` if valid, `Err(message)` if the path escapes the root.
fn assert_under_root(path: &str, root: &str) -> Result<(), String> {
